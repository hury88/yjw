...
$showtype || $showtype = v_news_cats($parentid,'showtype');
			...
			<div class="zhengwen clr">
				...
				<div class="miaoshu clr">
					<div id="tab1" class="tabson">
						...
						<input type="hidden" name="imgsize"  value="<?=v_news_cats($parentid,'imgsize')?>"  />
						...
						<?php if ($parentid<>0): ?>
						  <div class="layui-form-item">
							  ...
							  <div class="layui-input-block" style="line-height: 38px;">
							 	 <b><?php echo v_news_cats($parentid,'catname') ?></b>
							  	 <?php //=$dropdown?>
							  </div>
						 </div>
						<?php endif ?>
						<?php $opt
							->cache()->input('类别名称','catname')->verify('')->input('英文名称','catname2')->flur()->hide('showtype')

							->ifs(false)->display('inline')->input('手机品牌名称','catname')->img('配图','img1','193X126')
							->ifs($pid==3)->input('信息标题','seotitle')->textarea('信息简介','description')->endifs()
							// ->ifs($parentid==1)->display('inline')->img('配图','img1','193X126')
							->elses()
							->endifs()
							->ifs($parentid==8||$parentid==16)->img('配图','img1','420X170')->img('首页推荐图','img2','384X253')->textarea('列表简介', 'description')->endifs()
							->ifs(false)->img('目录图标','img2','69*67')->img('鼠标经过目录图标','img3','71*72')->endifs();
						?>
