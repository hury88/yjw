	...
	public function flur(){
		...
		switch ($this->in) {
			...
			case 'time':
			    ...
			    if ($isJson = $this->isJson()) {

			    	if ($isJson == 'jsononex') {
			    		$addButton = ''
			    		           .'<button style="margin-top: -6px;" class="layui-btn addbutton" onclick="var html=$(this).parent(\'.layui-form-item\').find(\'.layui-input-inline\').eq(0);var tmp = $(html.prop(\'outerHTML\'));tmp.find(\'input\').val(\'\');html.before(tmp);return false;">头部新增</button>'
				                   .'<button style="margin-top: -6px;" class="layui-btn addbutton" onclick="var html=$(this).parent(\'.layui-form-item\').find(\'.layui-input-inline\').last();var tmp = $(html.prop(\'outerHTML\'));tmp.find(\'input\').val(\'\');html.after(tmp);return false;">尾部新增</button>'
				                   .'<button style="margin-top: -6px;" class="deletebutton" onclick="var html=$(this).parent(\'.layui-form-item\');html.remove();return false;">x</button>'
				                   ;
			    	} else {

			    		$addButton = ''
			    		           .'<button style="margin-top: -6px;" class="layui-btn addbutton" onclick="var html=$(this).parent(\'.layui-form-item\');var tmp = $(html.prop(\'outerHTML\'));tmp.find(\'input\').val(\'\');html.before(tmp);return false;">向上新增+</button>'
				                   .'<button style="margin-top: -6px;" class="layui-btn addbutton" onclick="var html=$(this).parent(\'.layui-form-item\');var tmp = $(html.prop(\'outerHTML\'));tmp.find(\'input\').val(\'\');html.after(tmp);return false;">向下新增+</button>'
...
HTML;
	...
	//+-----------------------------------------------------------------
	public function jsonOneStart($labelName='',$inputName='', $jsonKeys=array())
	{
