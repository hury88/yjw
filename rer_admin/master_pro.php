...
//	if ($ty==21) {
//
//		$d2 = M('news')->where('pid=6 and ty=23')->order('disorder desc, isgood desc, id asc')->getField('id,title');Output::select($d2,'地区','istop');
//
//	} elseif ($ty==10) {
//
...
//	}
	...
	switch ($showtype) {
		...
		case 9://＜＞＜＞产品＜＞＜＞
			...
			(!isset($content) || ! $content ) && $content = v_news(6, -28, 'content', 29);
		...
		case 10://＜＞＜＞产品分类＜＞＜＞
			...
			$opt
                ...
                ->img('配图','img1')
                ;
	            // ->cache()->verify('float') ->input('宽', 'source') ->input('高', 'linkurl') ->flur()
//                ->jsonStart('下属分类', 'json', [
//            		    'json_catname' => '分类名称',
//            	    ])
//            	->jsonEnd();
		...
		case 11://＜＞＜＞图文列表＜＞＜＞
			...
			/*$phoneBrand = M('news_cats')->where('pid=3 and isstate=1')->order('id asc, disorder desc')->getField('id,catname');
			foreach ($phoneBrand as $phoneBrandId => $phoneBrandValue) {
				$phoneBrandModel = M('news')->where('ty='.$phoneBrandId.' and isstate=1')->order(config('other.order'))->getField('id,title');
				$opt->choose('<b style="color:#37a7e0;">'.$phoneBrandValue.'</b>','relative')->checkboxSet($phoneBrandModel)->flur();
			}*/
