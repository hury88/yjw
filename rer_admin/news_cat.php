...
<?php
	...
	$tree = new Tree(M('news_cats')->where('isstate =1')->order('id asc')->select());
 ...
 ?>
             ...
             <tr class="cat<?=$level?>">
                 ...
                 <td>
                 	<?php
                 		$spancer = array('','&emsp;├','&emsp;&emsp;└└');
                 		echo $spancer[$level];
                 	 ?>
                 	<a href="<?=getUrl($id,$showname.'_pro')?>" class="thick edits">编辑</a>
                 	<?php if (($level==0&&($v['iscats']==1)) || $pid==2): ?> <a href="<?=getUrl(array('pid'=>$id),$showname.'_pro')?>" class="thick" style="color:red">添加<?=$level+2?>级子栏目</a> <?php endif ?>

                 	<?php if ($level==2 &&($v['iscats']==1) ): ?> <a data-class="btn-danger" class="json <?=$v['isgood']==1?'btn-danger':'' ?>" data-url="isgood&id=<?=$id?>"><?=$webarr['isgood'][$v['isgood']] ?></a> <?php endif ?>

                 	<?php if ($level==1&&$v['iscats']==1): ?> <a href="javascript:;" data-id="<?=$id?>" data-opt="del" class="thick del">删除</a> <?php endif ?>

                 	<?php if ($level==2 || ($pid==2&&$level==1) ): ?><a data-class="btn-warm" class="json <?=$v['isstate']==1?'':'btn-warm' ?>" data-url="isstate&id=<?=$id?>"><?=$webarr['isstate'][$v['isstate']]?></a> <?php endif ?>
                 </td>
