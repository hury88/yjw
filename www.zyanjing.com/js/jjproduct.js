	...
	$.ajax({  // 获取所有品牌
        ...
        success:function(data){
        	for (var i=0;i<data.data.length;i++) {
        		box.append('<div class="list" data-index="'+data.data[i].index+'"><p><i class="add"></i><a href="javascript:;">'+data.data[i].value+'</a></p></list>');
        		if(data.data[i].index == allProArray[0]){
        			$.ajax({  // 获取某个品牌下面的所有系列
				        type:"POST",
				        ...
				        success:function(data){
				        	var li = '';
				        	for (var i=0;i<data.data.length;i++) {
				        		li += '<li data-index="'+data.data[i].index+'">'+data.data[i].value+'</li>';
				        	}
        ...
        $.ajax({  // 获取某个品牌下面的所有系列
            ...
            success:function(data){
                ...
                if(data.data.length>0){
                    for (var i=0;i<data.data.length;i++) {
                        jj_options.append('<a href="javascript:;" data-index="'+data.data[i].index+'">'+data.data[i].value+'</a>');
                ...
                if(data.data.length>0) {
                    for (var i = 0; i < data.data.length; i++) {
                        if(data.data[i].value!=null&&data.data[i].value!="") {
                            $(".kuangxing").show();
                            kuangxing.append('<a href="javascript:;" data-value=' + data.data[i].index + '>' + data.data[i].value + '</a>');
                        }
                    }
                }else {
                    $(".kuangxing").hide();
            ...
            success:function(data){
                ...
                if(data.data.length>0) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        if(data.data[i].value!=null&&data.data[i].value!="") {
                             $(".kuanshi").show();
                            kuanshi.append('<a href="javascript:;" data-value=' + data.data[i].index + '>' + data.data[i].value + '</a>');
                        }
                    }
                }else {
                    $(".kuanshi").hide();
